...
class BaseLearner(object):
    ...
    def tsne(self, showcenters=False, Normalize=False):
        ...
        if showcenters:
            fc_weight = self._network.fc.proj.cpu().detach().numpy()[
                :tot_classes]
        ...
        scatter = plt.scatter(
            embedding[:, 0], embedding[:, 1], c=y_true, s=20, cmap=plt.cm.get_cmap("tab20"))
    ...
    def _evaluate(self, y_pred, y_true):
        ...
        grouped = accuracy(y_pred.T[0], y_true,
                           self._known_classes, self.args["increment"])
        ret["grouped"] = grouped
