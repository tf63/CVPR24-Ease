...
class Learner(BaseLearner):
        ...
        with torch.no_grad():
            ...
            for index in range(start_idx, self._cur_task + 1):
                ...
                for class_index in class_list:
                    data_index = (
                        label_list == class_index).nonzero().squeeze(-1)
                    ...
                    if self.use_init_ptm:
                        ...
                        model.fc.weight.data[class_index, index * self._network.out_dim:(
                            index + 1) * self._network.out_dim] = proto

            if self.use_exemplars and self._cur_task > 0:
                embedding_list = []
                label_list = []
                dataset = self.data_manager.get_dataset(
                    np.arange(0, self._known_classes), source="train", mode="test", )
                loader = DataLoader(
                ...
                for i, batch in enumerate(loader):
                    ...
                    label = label.to(self._device)
                    embedding = model.backbone.forward_proto(
                        data, adapt_index=self._cur_task)
                    embedding_list.append(embedding.cpu())
                    label_list.append(label.cpu())
                embedding_list = torch.cat(embedding_list, dim=0)
                label_list = torch.cat(label_list, dim=0)
    ...
    def get_A_B_Ahat(self, task_id):
        ...
        A = self._network.fc.weight.data[self._known_classes:,
                                         start_dim: end_dim]
        # W(TT)
        B = self._network.fc.weight.data[self._known_classes:, -
                                         self._network.out_dim:]
        # W(ii)
        A_hat = self._network.fc.weight.data[start_cls: end_cls,
                                             start_dim: end_dim]
    ...
    def solve_similarity(self):
        for task_id in range(self._cur_task):
            ...
            # B_hat(old_cls2)
            self._network.fc.weight.data[start_cls: end_cls, -
                                         self._network.out_dim:] = B_hat.to(self._device)
    ...
    def solve_sim_reset(self):
        for task_id in range(self._cur_task):
            ...
            for dim_id in range_dim:
                ...
                # B_hat(old_cls2)
                self._network.fc.weight.data[start_cls: end_cls, start_dim: end_dim] = B_hat.to(
                    self._device)
    ...
    def incremental_train(self, data_manager):
        ...
        self.train_dataset = data_manager.get_dataset(np.arange(
        ...
        self.train_dataset_for_protonet = data_manager.get_dataset(np.arange(
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self._cur_task == 0 or self.init_cls == self.inc:
            ...
            scheduler = self.get_scheduler(
                optimizer, self.args["later_epochs"])
    ...
    def _eval_cnn(self, loader):
        ...
        for _, (_, inputs, targets) in enumerate(loader):
            ...
            if calc_task_acc:
                ...
                for i, task_id in enumerate(task_ids):
                    ...
                    task_logits[i, start_cls:end_cls] += outputs[i,
                                                                 start_cls:end_cls]
                # calculate the accuracy of task_id
                pred_task_ids = (torch.max(outputs, dim=1)[
                                 1] - self.init_cls) // self.inc + 1
        ...
        if calc_task_acc:
            logging.info("Task correct: {}".format(
                tensor2numpy(task_correct) * 100 / total))
            logging.info("Task acc: {}".format(
                tensor2numpy(task_acc) * 100 / total))
